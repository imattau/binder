...
export const boostService = {
    async getBoosts(eventCoords: { kind: number, pubkey: string, d: string }): Promise<Result<NostrEvent[]>> {
        ...
        try {
            ...
            return ok(events);
             ...
             return fail({ message: 'Failed to fetch boosts', cause: e });
        }
    },

    async boost(eventCoords: { kind: number, pubkey: string, d: string }): Promise<Result<void>> {
        const relays = await getActiveRelays();
        ...
        try {
            ...
            return fail({ message: 'Failed to publish boost', cause: e });
        }
    }
};
